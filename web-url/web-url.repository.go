...
func (webUrlRepository *WebUrlRepository) GetWebUrls() (groupedWebUrls map[int][]WebUrl, err error) {
	var logger = webUrlRepository.logger
	var webUrls []WebUrl
	result := webUrlRepository.database.Find(&webUrls)
	if result.Error != nil {
		logger.Error("Error while fetching web urls", zap.Error(result.Error))
	...
	logger.Info("Grouped web urls", zap.Any("groupedWebUrls", groupedWebUrls))
...
func (webUrlRepository *WebUrlRepository) GetAllDistinctWebUrlIntervals() (intervals []int, err error) {
	...
	if result.Error != nil {
		logger.Error("Error while fetching intervals", zap.Error(result.Error))
		return nil, result.Error
	}
	logger.Info("Distinct intervals", zap.Any("intervals", intervals))
...
func (webUrlRepository *WebUrlRepository) GetWebUrlsByInterval(interval int) (webUrls []WebUrl, err error) {
	...
	logger.Info(fmt.Sprintf("Web urls for interval %d", interval), zap.Any("webUrls", webUrls))
	return webUrls, nil
}
